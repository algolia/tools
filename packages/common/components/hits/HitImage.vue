    ...
    >
        <div
            ...
            :class="[
                ...
                `w-${forcedSize || imageSize} h-${forcedSize || imageSize}`
            ]"
        >
            ...
            <img
                ...
                :src="`https://rocky-thicket-17824.herokuapp.com/${forcedSize || imageSize}/${imageBaseUrl}${image}${imageSuffixUrl}`"
            />
        </div>
        <div v-if="editImageAttributes && !forcedSize" class="p-12 text-center">
            ...
            <div class="mt-2">
                <input class="w-128 bg-proton-grey-opacity-20 text-xs p-4" v-model="imageBaseUrl" placeholder="base_image_url" />
            </div>
            <div class="mt-2">
                <input class="w-128 bg-proton-grey-opacity-20 text-xs p-4" v-model="imageSuffixUrl" placeholder="suffix_image_url" />
            </div>
            <div class="mt-2">
                <select v-model.number="imageSize">
...
<script>
    ...
    export default {
        name: 'HitImage',
        props: ['panelKey', 'flattenHit', 'displayMode', 'listMode', 'forcedSize', 'imageSize', 'imageBaseUrl', 'imageSuffixUrl'],
