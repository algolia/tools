<template>
  <div class="mt-24">
    <div v-if="!tasksGroup && !displayCopyOption" class="flex">
      <button v-if="!readOnly && isIndexSettingsDirty"
            ...
            <div>
              <label>
                ...
                <span v-if="limitCopy.enabled">
                  : <input v-model="limitCopy.nbHits" class="input-custom inline w-72" type="number" min="0"
                    step="1000">
                </span>
              </label>
            </div>
            <div v-if="!sameAppCopyMethod">
              <label>
                ...
                <span v-if="configureBatchSize">
                  : <input v-model="batchSize" class="input-custom inline w-72" type="number" min="1" max="1000"
                    step="100">
                </span>
              </label>
            </div>
            ...
            <div>
              <label>
                ...
                <span v-if="overwriteDefaultTimeout">
                  : <input v-model="writeTimeout" class="input-custom inline w-72" type="number" min="30" max="1000"
                    step="10"> secs
                </span>
              </label>
            </div>
  ...
  methods: {
    ...
    if (!this.limitCopy.currentQueryOnly) {
        ...
        browseTask.setCallback(async () => {
            ...
            let res = await config.srcIndex.customBrowse(params);
            let nbToCopy = this.limitCopy.enabled ? Math.min(res.nbHits, this.limitCopy.nbHits) : res.nbHits;
            nbCopied += res.hits.length;
            browseTask.setNth(0);
            browseTask.setOutOf(Math.ceil(nbToCopy / config.hitsPerPage));
            ...
            while (res.cursor && (!this.limitCopy.enabled || nbCopied < this.limitCopy.nbHits)) {
                ...
                resAdd = await config.dstIndex.saveObjects(res.hits.map((hit) => {
                    delete(hit._highlightResult);
                    delete(hit._snippetResult);
                    return hit;
                }));
        ...
        tasksGroup.addTask(browseTask); // Add browseTask to tasksGroup here
    }
}
      ...
      waitTask.setCallback(async () => {
        if (tasksGroup.tasksIds.length > 0) {
          ...
          for (let i = 0; i < tasksGroup.tasksIds.length; i++) {
